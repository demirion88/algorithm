...
#하지만, 김탑은 대각선의 길이가 같다고 해도, 실제 TV의 크기는 다를 수도 있다는 사실에 직접 TV를 보러갈걸 왜 인터넷 쇼핑을
#대각선의 길이만 보고있는지 후회하고 있었다.
...
def calc(length,width, height):
    ratio = length / math.sqrt(width**2 + height**2)
    lengthwh = [int(ratio * width), int(ratio * height)]
    return lengthwh

print(
    str(calc(length, width, height)).replace(",","")[1:-1]
         )